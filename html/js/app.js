        ...
        if (CanOpenApp) {
            if (MI.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $("#playerName").text(MI.Phone.Data.PlayerData.Name)
                    $("#playerNames").text(MI.Phone.Data.PlayerData.Name)
...
MI.Phone.Functions.UpdateTime = function(data) {    
    ...
    var PhoneTime = Hourssssss + " " + Minutessss
    var gunler = ["Pzr", "Pzt", "Slı", "Çrş", "Prş", "Cma", "Cts"];
    var bugun = gunler[NewDate.getDay()];
    var tarih = new Date();
    var kacinciGun = tarih.getDate();
    $("#phone-time").html(MessageTime);
    $("#saat-bilgi").html(PhoneTime);
    $("#saat-gun").html(bugun);
    $("#saat-aygun").html(kacinciGun);
}
