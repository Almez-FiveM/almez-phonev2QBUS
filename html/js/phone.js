                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (MI.Phone.Data.AnonymousCall) {
                            MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"), MI.Phone.Functions.Lang("PHONE_STARTED_ANON"));
                        }
                        ...
                        setTimeout(function(){
                            ...
                            MI.Phone.Animations.FadeIn('.phone-application-container', 400, 0);
                        ...
                        MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"), MI.Phone.Functions.Lang("PHONE_BUSY"));
                    }
                } else {
                    MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"), MI.Phone.Functions.Lang("PHONE_PERSON_TALKING"));
                }
            } else {
                MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"), MI.Phone.Functions.Lang("PHONE_PERSON_UNAVAILABLE"));
            }
        } else {
            MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"), MI.Phone.Functions.Lang("PHONE_YOUR_NUMBER"));
        }
    });
});
            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"),"valla çok bussy");
                    }
                } else {
                    MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"), "konsuyuro");
                }
            } else {
                MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"), MI.Phone.Functions.Lang("PHONE_PERSON_UNAVAILABLE"));
            }
        } else {
            MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"), "phone esittir phone");
        }
    });
});
...
MI.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    $("#contact-search").on("keydown", function() {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            var ContactElement = '<div class="phone-contact" data-contactid="'+i+'"><div class="phone-contact-firstletter" style="background-color: #e74c3c;">'+((contact.name).charAt(0)).toUpperCase()+'</div><div class="phone-contact-name">'+contact.name+'</div><div class="phone-contact-action-buttons"> <i class="fas fa-phone-volume" id="phone-start-call"></i> <i class="fab fa-whatsapp" id="new-chat-phone" style="font-size: 2.5vh;"></i> <i class="fas fa-user-edit" id="edit-contact"></i> </div></div>'
            if (contact.status) {
                ContactElement = '<div class="phone-contact" data-contactid="'+i+'"><div class="phone-contact-firstletter" style="background-color: #2ecc71;">'+((contact.name).charAt(0)).toUpperCase()+'</div><div class="phone-contact-name">'+contact.name+'</div><div class="phone-contact-action-buttons"> <i class="fas fa-phone-volume" id="phone-start-call"></i> <i class="fab fa-whatsapp" id="new-chat-phone" style="font-size: 2.5vh;"></i> <i class="fas fa-user-edit" id="edit-contact"></i> </div></div>'
            }
    ...
    var a = document.querySelectorAll('[data-contactid]');

    $.each(a, function(i){
        console.log()
        $("[data-contactid='" + i + "']").on('click', function() {
            console.log("tikladim" + i)
            LoadUserContact(myContacts[i],i)
            MI.Phone.Animations.SoldanSaga('.phone-contact-user');
            console.log("tikladigimin ismi: " + myContacts[i].number)
        });
    ...
    // Tüm contactleri alıcak ve listleyip ilk harfini döngünün içindeki bir değişkene tanımlayacak.
    
    // Daha sonra kaç tane ilk harf olduğunu bir tabloya ekliycek.

    // Sonra eğer tabloda bir harf varsa onun hr elementini oluşturacak.

};

$(document).on('click', '#phone-send-message', function(e){
    var ContactName = $(".phone-contact-user-name").val();
    var ContactNumber = document.getElementById("phone-contact-user-number").innerHTML;
    console.log(ContactName)
    console.log('ASD' + ContactName)
    if (ContactNumber !== MI.Phone.Data.PhoneNumber) {
        ...
        setTimeout(function(){
            ...
            $.post('http://almez-phonev2/GetWhatsappChat', JSON.stringify({phone: ContactNumber}), function(chat){
...
LoadUserContact = function(contactuser,id) {
    $.post('http://almez-phonev2/GetContactsPicture', JSON.stringify({"number" : contactuser.number}), function(data){
        $.each(data, function(i, ppicture){
            var profilepicture = "./img/default.png";
            if (ppicture !== "default") {
                profilepicture = ppicture
                console.log(ppicture);
            }
            $("#phone-contact-user").find('.phone-contact-user-icon-in').html('<img class="phone-contact-user-icon-img" src="' + profilepicture + '">');
            
        });
    });

    console.log("id: " + id);
    $('#phone-contact-user').attr("data-usercontactid",id);
    console.log(" datası: " + $("#phone-contact-user").data('usercontactid'))
    $(".phone-contact-user-name").text(contactuser.name);
    $(".phone-contact-user-number-block-text").html(contactuser.number)
    $(document).on('mouseup', '#phone-start-call', function(e){
        e.preventDefault();   

        var ContactId = id;
        console.log("aramaya bastım contacd id: " + id);
        var ContactData = GetContactByID(id);
...
$(document).on('click', '#edit-contact', function(e){
    ...
    MI.Phone.Animations.FadeIn(".phone-edit-contact", 200, 0);
});

$(document).on('click', '#back-contact-list', function(e){
    e.preventDefault();
    MI.Phone.Animations.SagdanSola(".phone-contact-user");
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        MI.Phone.Notifications.Add("phone", MI.Phone.Functions.Lang("PHONE_TITLE"), MI.Phone.Functions.Lang("ALLFIELDS"));
    }
});
...
$(document).on('click', '.phone-keypad-key', function(e){
    ...
    if(keyPadHTML == ""){
        $(".phone-keypad-key-delete").css({"display":"block"}).animate({
            opacity: "1",
        }, 150);

        $("#phone-keypad-new-contact").css({"display":"inline"}).animate({
            opacity: "1",
        }, 150);
    }
    if ($("#phone-keypad-input").text().length <= 10){
...
$(document).on('click', '.phone-keypad-key-delete', function(e){
    e.preventDefault();

    var keyPadHTML = $("#phone-keypad-input").text();
    
    if (keyPadHTML.length != null) {
        let keyPadLenght = keyPadHTML.length;
        keyPadHTML = keyPadHTML.slice(0, keyPadLenght - 1);
        $("#phone-keypad-input").text(keyPadHTML);
    }

    if($("#phone-keypad-input").text().length === 0){
        $(".phone-keypad-key-delete").css({"display":"block"}).animate({
            opacity: "0",
        }, 150, function(){
            $(".phone-keypad-key-delete").css({"display":"none"});
        });

        $("#phone-keypad-new-contact").css({"display":"inline"}).animate({
            opacity: "0",
        }, 150, function(){
            $("#phone-keypad-new-contact").css({"display":"none"});
        });
    }
...
$(document).on('click', '#phone-keypad-new-contact', function(e){
    e.preventDefault();

    var NewContactNumber = $("#phone-keypad-input").text();
    
    MI.Phone.Animations.FadeIn(".phone-add-contact", 200, 0);
    $(".phone-add-contact-number").val(NewContactNumber);
    console.log($(".phone-add-contact-number").val());
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    MI.Phone.Animations.FadeIn('.phone-application-container', 500, 0);
    MI.Phone.Animations.FadeIn('.phone-call-app', 500, 0);
    MI.Phone.Functions.ToggleApp("phone-call", "block");
